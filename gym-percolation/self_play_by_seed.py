...
def get_model_action(action_space, observation, model, exp_rate=0.1):
    ...
    if exp_rate == 0:
        return return_action, action_space
    ...
    if rand <= exp_rate and not True in np.isnan(probs):
        return_action = np.random.choice(action_space)
    else:
        return_action = np.random.choice(action_space, p=probs)
    ...
    while not done:
        ...
        if len(action_space) > 0:
            if params['random'] == True:
                ...
                action, action_space = get_model_action(action_space, observation, params['model'], params["exp_rate"])
            action_step = {}
            observation, reward, done, info = env.step(action)
            action_step["board_state"] = observation.tolist()
            action_step["action_space"] = action_space
